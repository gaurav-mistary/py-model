...
class ImmutableModel(MutableModel):
    ...
    def _copy(self, key: str, value: T) -> Self:
        """
        ...
        try:
            ...
            if (_obj := self.model_fields.get(model_attr_name)) is None:
                raise AttributeError(
                    f"[X] Field: {key} is missing attribute: {model_attr_name}"
                )

            if not hasattr(_obj.default, "model_copy"):
                raise ValueError(
                    f"[X] Key: {key} includes a non-pydantic model class "
                    f"attribute: {model_attr_name}, with class: {_obj.default.__class__}"
                )

            return self._copy(
                key=".".join(key_split[:-1]),
                value=_obj.default.model_copy(update={attr_name: value}),
            )

        except IndexError:
            model_attr_name = key_split.pop(0)
            _obj = self.model_fields.get(model_attr_name)
            return self.model_copy(update={model_attr_name: value})

    def new(self, key: str, value: T) -> Self:
        """
        This is probably my best contribution to recursive coding,
        The new method series on internal _copy method which in-turn relies on Model.model_copy method
        ...
        :return:
        """
        return self._copy(key=key, value=value)
